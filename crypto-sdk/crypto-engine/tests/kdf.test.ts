...
describe("MasterKey", () => {
  it("derives deterministically for the same context", () => {
    ...
    const a = master.deriveSymmetricKey({
      context: "session",
      purpose: DerivationPurpose.ENCRYPTION,
    });
    const b = master.deriveSymmetricKey({
      context: "session",
      purpose: DerivationPurpose.ENCRYPTION,
    });
  ...
  it("separates keys by context, purpose, and version", () => {
    ...
    expect(
      master.deriveBytes({ context: "file", purpose: DerivationPurpose.ENCRYPTION }),
    ).not.toEqual(base);
    expect(master.deriveBytes({ context: "session", purpose: DerivationPurpose.MAC })).not.toEqual(
      base,
    );
    expect(
      master.deriveBytes({ context: "session", purpose: DerivationPurpose.ENCRYPTION, version: 2 }),
    ).not.toEqual(base);
  });

  it("supports hierarchical sub-master keys", () => {
    const root = MasterKey.random();
    const child = root.deriveMasterKey({
      context: "subsystem",
      purpose: DerivationPurpose.GENERIC,
    });
  ...
  it("builds from shared secret and password", () => {
    ...
    expect(
      MasterKey.fromSharedSecret(secret).deriveSymmetricKey({ context: "c", purpose: "p" }).length,
    ).toBe(32);
...
describe("deriveSessionKey (from shared secret)", () => {
  it("both peers derive the same session key for the same context", () => {
    ...
    expect(deriveSessionKey(sA, "peer-42").bytes).not.toEqual(
      deriveSessionKey(sA, "peer-99").bytes,
    );
  });
});
...
describe("buildInfoLabel", () => {
  it("encodes namespace:context:purpose:version", () => {
    const label = new TextDecoder().decode(
      buildInfoLabel({ context: "session", purpose: "encryption" }),
    );
