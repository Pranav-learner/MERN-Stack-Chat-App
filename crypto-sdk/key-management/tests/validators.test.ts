...
describe("validators", () => {
  ...
  it("rejects invalid metadata fields", () => {
    ...
    expect(() => validator.validateMetadata({ ...key.metadata, keyId: "" })).toThrow(KeyValidationError);
    expect(() => validator.validateMetadata({ ...key.metadata, version: 0 })).toThrow(KeyValidationError);
    expect(() => validator.validateMetadata({ ...key.metadata, rotationCount: -1 })).toThrow(KeyValidationError);
    // @ts-expect-error invalid enum at runtime
    expect(() => validator.validateMetadata({ ...key.metadata, type: "nope" })).toThrow(KeyValidationError);
    // @ts-expect-error invalid enum at runtime
    expect(() => validator.validateMetadata({ ...key.metadata, status: "nope" })).toThrow(KeyValidationError);
    expect(() => validator.validateMetadata({ ...key.metadata, createdAt: "not-a-date" })).toThrow(
      KeyValidationError,
    );
    expect(() => validator.validateMetadata({ ...key.metadata, fingerprint: "" })).toThrow(KeyValidationError);
  });
  ...
  it("can skip the fingerprint check", () => {
    ...
    expect(() => validator.validateManagedKey(corrupted, { checkFingerprint: false })).not.toThrow();
  });

  it("enforces expiry when asked", () => {
    const key = makeIdentityKey().withMetadata({ expiresAt: toIso(1000), status: KeyStatus.ACTIVE });
    ...
    expect(() => validator.validateManagedKey(key, { checkExpiry: true, now: 2000 })).toThrow(KeyExpiredError);
  });
