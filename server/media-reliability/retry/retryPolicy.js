/**
 * @module media-reliability/retry
 *
 * **Configurable retry policies** for media-operation recovery. Supports immediate retry, exponential
 * backoff (with deterministic jitter), a fixed delay, a bounded max-attempt count, a lifetime retry
 * BUDGET, a cooldown, automatic resume, manual retry, and per-call policy overrides. Pure functions + a
 * small policy resolver — no I/O.
 *
 * @distributed A retry budget (total retries across a record's life) bounds the cost of a persistently
 * failing transfer so it can't retry-storm forever; the max-attempt count bounds a single recovery
 * episode. This is what stops a broken upload (e.g. a permanently failing storage provider) from
