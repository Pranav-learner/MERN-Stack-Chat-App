/**
 * @module media-reliability/manager/lifecycle
 *
 * The deterministic finite state machine governing a media operation's RELIABILITY lifecycle (distinct
 * from the Sprint-1/2 operation states — those track an operation's own progress; this tracks continuity
 * + health). Pure logic — no I/O. The {@link module:media-reliability/manager manager} validates every
...
export function assertTransition(from, to) {
  ...
  if (!canTransition(from, to)) throw new InvalidReliabilityTransitionError(`Cannot transition media operation from "${from}" to "${to}"`, { details: { from, to, allowed: ALLOWED_TRANSITIONS[from] ?? [] } });
}
