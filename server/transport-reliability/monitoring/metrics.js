/**
 * @module transport-reliability/monitoring/metrics
 *
 * **Production metrics registry** for the Data Plane. A dependency-free, in-process registry of
 * counters, gauges, and histograms, with `snapshot()`, a Prometheus text-exposition renderer, and an
 * OpenTelemetry export hook. Wire it into the reliability manager to record transfer throughput +
 * latency, average chunk size, transfer success rate, retry + resume counts, queue length, outstanding
 * chunks, backpressure events, concurrent transfers, recovery/migration counts, and health score.
 *
 * @security Metrics are numeric aggregates + low-cardinality labels — NEVER key material, payload
 * bytes, or per-transfer ids as labels.
 */
...
export class TransferMetrics {
  ...
  // === transport-specific recorders ========================================

  /** Record a completed/failed transfer (total + success/failure) + its latency. @returns {number} success rate */
  recordTransfer(success, latencyMs) {
    this.increment(Metric.TRANSFER_TOTAL);
    this.increment(success ? Metric.TRANSFER_SUCCESS : Metric.TRANSFER_FAILURE);
    if (Number.isFinite(latencyMs)) this.observe(Metric.TRANSFER_LATENCY, latencyMs);
    return this.transferSuccessRate();
  }
  ...
  recordMigration(success) {
    this.increment(Metric.MIGRATION_TOTAL);
    if (success) this.increment(Metric.MIGRATION_SUCCESS);
  }
  ...
  /** Record throughput (bytes/sec) + an observed chunk size (bytes). */
  recordThroughput(bytesPerSec, chunkSizeBytes) {
    if (Number.isFinite(bytesPerSec)) this.observe(Metric.TRANSFER_THROUGHPUT, bytesPerSec);
    if (Number.isFinite(chunkSizeBytes)) this.observe(Metric.CHUNK_SIZE, chunkSizeBytes);
  }

  /** The transfer success rate `[0,1]`. */
  transferSuccessRate() {
    const s = this._val(Metric.TRANSFER_SUCCESS);
    const f = this._val(Metric.TRANSFER_FAILURE);
  ...
  snapshot() {
    ...
    return { counters, gauges, histograms, transferSuccessRate: this.transferSuccessRate(), recoverySuccessRate: this.recoverySuccessRate() };
  }
  ...
  exportMetrics() {
    ...
    for (const fn of this._exporters) {
      try {
        ...
        /* an exporter failure must never break the transport path */
      }
    }
    return snap;
  }
