/**
 * @module synchronization-reliability/recovery
 *
 * The **Recovery Engine** — decides HOW to recover an interrupted synchronization and executes the
 * recovery via INJECTED hooks. It maps a trigger (interrupted-sync / device-crash / app-restart /
 * connection-loss / partial-sync / transfer-failure / stall) to an action (resume-from-checkpoint /
 * retry / restart / graceful-fail), enforces bounded attempts + a retry budget + a total recovery
 * timeout, and always PRESERVES replica consistency by resuming from the monotonic checkpoint.
 *
 * @important Recovery NEVER corrupts replica state: it is a pure decision + a hook call; the checkpoint
 * is read, never mutated, and the manager applies state transitions atomically. On exhaustion the
 * synchronization fails GRACEFULLY with its checkpoint intact (a caller can resume later).
 *
 * @evolution Transport-INDEPENDENT: the actual re-run is the Sprint 1/2 engines' job (+ Layer-8 for
 * attachment transfers), injected here as hooks. Defaults are optimistic (the device performs the work
 * + confirms via a subsequent checkpoint).
 */
...
export class RecoveryCoordinator {
   ...
   *
   * @param {object} params
   * @param {import("../types/types.js").SyncReliabilityRecord} params.record
   * @param {string} params.trigger @param {number} params.attempt (1-based)
   * @param {object} params.hooks `{ resumeFromCheckpoint, retry, restart, gracefulFail }` (async → boolean)
  ...
  async run({ record, trigger, attempt, hooks = {} }) {
    ...
    const resumePlan = planResume(record.checkpoint ?? { totalOperations: 0 }, { now: this.clock() });
    ...
    switch (action) {
      ...
      case RecoveryAction.RESTART: {
        const ok = await safe(hooks.restart, record, resumePlan);
        return { outcome: ok ? RecoveryOutcome.RECOVERED : RecoveryOutcome.FAILED, action, resumePlan, recoverable: true };
      }
