/**
 * @module group-reliability/manager/lifecycle
 *
 * The deterministic finite state machine governing a group operation's RELIABILITY lifecycle (distinct
 * from the Sprint-2 delivery/fan-out states — those track an operation's own progress; this tracks
 * continuity + health). Pure logic — no I/O. The {@link module:group-reliability/manager manager}
 * validates every transition through here.
 *
 * ```mermaid
 * stateDiagram-v2
 *   [*] --> tracking
...
export function assertTransition(from, to) {
  ...
  if (!canTransition(from, to)) throw new InvalidReliabilityTransitionError(`Cannot transition group operation from "${from}" to "${to}"`, { details: { from, to, allowed: ALLOWED_TRANSITIONS[from] ?? [] } });
}
