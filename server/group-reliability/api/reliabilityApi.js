/**
 * @module group-reliability/api
 *
 * The stable **group-reliability service facade** the HTTP controller delegates to. Wraps the
 * {@link GroupReliabilityManager} with the observability + hardening surface: register / checkpoint /
 * recover / resume / complete / abandon, plus read-only health (per-operation + per-group), metrics
...
import { protocolManifest, isGroupLayerCompatible, EXTENSION_POINTS } from "../freeze/protocolFreeze.js";
import { auditGroupApis, SECURITY_ASSUMPTIONS, normalizePagination } from "../security/securityAudit.js";

export function createGroupReliabilityApi(manager, deps = {}) {
  ...
  return {
    ...
    getGroupHealth: ({ groupId }) => manager.getGroupHealth(groupId),
    getDiagnostics: ({ operationId, actingDevice }) => manager.getDiagnostics(operationId, { actingDevice }),
    listOperations: ({ groupId, userId, state, limit }) => manager.listOperations({ groupId, userId, state, limit }),
    ...
    auditTrail: async ({ groupId, limit } = {}) => {
      if (!auditStore) return [];
      return groupId ? auditStore.listByGroup(groupId, { limit }) : auditStore.list({ limit });
    },

    // freeze + security
    protocol: () => ({ ...protocolManifest, compatible: isGroupLayerCompatible(protocolManifest.versions.groupLayer), extensionPoints: EXTENSION_POINTS }),
    securityAudit: () => ({ ...auditGroupApis(), assumptions: SECURITY_ASSUMPTIONS }),
  };
}
