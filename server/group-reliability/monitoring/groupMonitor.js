/**
 * @module group-reliability/monitoring/monitor
 *
 ...
 * spikes, repeated recovery failures, unhealthy groups, stall timeouts, high fan-out failure, high
 * replica drift, large offline backlogs, and retry storms. Alerts are persisted via an injected sink +
...
export class GroupMonitor {
  ...
  _onEvent(event) {
    switch (event.type) {
      case ReliabilityEventType.OPERATION_FAILED:
        this.track("operation-failure", AlertType.OPERATION_FAILURE_SPIKE, AlertSeverity.CRITICAL, { operationId: event.operationId, groupId: event.groupId });
        break;
      ...
      case ReliabilityEventType.BACKLOG_DETECTED:
        this.track("backlog", AlertType.LARGE_OFFLINE_BACKLOG, AlertSeverity.WARNING, { operationId: event.operationId, backlog: event.backlog });
        break;
      case ReliabilityEventType.HEALTH_CHANGED:
        if (event.status === "unhealthy") this.track("unhealthy", AlertType.UNHEALTHY_GROUP, AlertSeverity.WARNING, { operationId: event.operationId, groupId: event.groupId });
        if (event.failureRate != null && event.failureRate >= 0.5) this.track("fanout-failure", AlertType.HIGH_FANOUT_FAILURE, AlertSeverity.WARNING, { operationId: event.operationId, failureRate: event.failureRate });
        break;
      default:
        break;
    }
  ...
  _raise(type, severity, details) {
    ...
    try {
      ...
      /* alert persistence failure must never break the group path */
    }
    return alert;
  }
