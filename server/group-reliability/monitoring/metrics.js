/**
 * @module group-reliability/monitoring/metrics
 *
 * **Production metrics registry** for the Group Communication platform. A dependency-free, in-process
 * registry of counters, gauges, and histograms, with `snapshot()`, a Prometheus text-exposition
 * renderer, and an OpenTelemetry export hook. Wire it into the reliability manager to record messages
 * per group, fan-out latency + target counts, group throughput, replica drift, synchronization latency,
 * membership changes, key-rotation count, offline queue size, pending operations, recovery success rate,
 * resume + retry counts, concurrent operations, and health score.
 *
 * @security Metrics are numeric aggregates + low-cardinality labels — NEVER content, per-member ids as
...
export class GroupMetrics {
  ...
  // === group-specific recorders ============================================

  /** Record a completed/failed group operation (total + success/failure). */
  ...
  /** Record a group message send + its fan-out (targets + latency). */
  recordMessage({ groupId, targets, latencyMs } = {}) {
    this.increment(Metric.MESSAGES_PER_GROUP, 1, groupId ? { group: groupId } : undefined);
    if (Number.isFinite(targets)) this.observe(Metric.FANOUT_TARGETS, targets);
    if (Number.isFinite(latencyMs)) this.observe(Metric.FANOUT_LATENCY, latencyMs);
  }
  ...
  recordMembershipChange() {
    this.increment(Metric.MEMBERSHIP_CHANGES);
  }
  recordKeyRotation() {
    this.increment(Metric.KEY_ROTATION_TOTAL);
  }
  ...
  /** Record per-operation gauges (throughput, replica drift, pending, offline queue). */
  recordProgress({ throughput, replicaDrift, pendingOperations, offlineQueueSize } = {}) {
    if (Number.isFinite(throughput)) this.observe(Metric.GROUP_THROUGHPUT, throughput);
    ...
    if (Number.isFinite(offlineQueueSize)) this.gauge(Metric.OFFLINE_QUEUE_SIZE, offlineQueueSize);
  }

  operationSuccessRate() {
    const s = this._val(Metric.OPERATION_SUCCESS);
    const f = this._val(Metric.OPERATION_FAILURE);
  ...
  snapshot() {
    ...
    return { counters, gauges, histograms, operationSuccessRate: this.operationSuccessRate(), recoverySuccessRate: this.recoverySuccessRate() };
  }
  ...
  exportMetrics() {
    ...
    for (const fn of this._exporters) {
      try {
        ...
        /* an exporter failure must never break the group path */
      }
    }
    return snap;
  }
